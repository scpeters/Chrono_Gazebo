...
IF( ChronoEngine_FIND_COMPONENTS )
    ...
    ##### The unit_VEHICLE COMPONENT

    if (${CH_USE_UNIT_VEHICLE})


        FIND_LIBRARY(CHRONOENGINE_LIBRARY_VEHICLE_RELEASE
          NAMES ChronoEngine_Vehicle ChronoEngine_VehicleUtils
          PATHS ${CH_LIBDIR_RELEASE}
          )
        FIND_LIBRARY(CHRONOENGINE_LIBRARY_VEHICLE_DEBUG
          NAMES ChronoEngine_Vehicle ChronoEngine_VehicleUtils
          PATHS ${CH_LIBDIR_DEBUG}
          )
        #MESSAGE( STATUS " Finding unit_VEHICLE component!" )
        SET(CHRONOENGINE_LIBRARY_VEHICLE "")
        IF(CHRONOENGINE_LIBRARY_VEHICLE_RELEASE)
          SET(CHRONOENGINE_LIBRARY_VEHICLE
            ${CHRONOENGINE_LIBRARY_VEHICLE}
            optimized ${CHRONOENGINE_LIBRARY_VEHICLE_RELEASE}
            )
        ENDIF()
        IF(CHRONOENGINE_LIBRARY_VEHICLE_DEBUG)
          SET(CHRONOENGINE_LIBRARY_VEHICLE
            ${CHRONOENGINE_LIBRARY_VEHICLE}
            debug ${CHRONOENGINE_LIBRARY_VEHICLE_DEBUG}
            )
        ENDIF()

        # Hide uneeded stuff from GUI.
        MARK_AS_ADVANCED(CHRONOENGINE_LIBRARY_VEHICLE_RELEASE)
        MARK_AS_ADVANCED(CHRONOENGINE_LIBRARY_VEHICLE_DEBUG)

        # Append to easy collective variables
        SET(CH_INCLUDES ${CH_INCLUDES} "${CH_CHRONO_SDKDIR}/unit_VEHICLE" )
        SET(CHRONOENGINE_INCLUDES  ${CH_INCLUDES})
        SET(CHRONOENGINE_LIBRARIES ${CHRONOENGINE_LIBRARIES} ${CHRONOENGINE_LIBRARY_VEHICLE})
    endif()
    ...
    if (${CH_USE_UNIT_IRRLICHT})
        #MESSAGE( STATUS "For some reason, it thinks it is trying to find IRRLICHT!")
        IF(${CMAKE_SYSTEM_NAME} MATCHES "Windows")
